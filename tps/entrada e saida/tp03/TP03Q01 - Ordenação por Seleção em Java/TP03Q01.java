...
class Lista {
  ...
  void ordenarSelecao() throws Exception {
    if (n == 0) {
      throw new Exception("ERRO");
    }
    ...
    for (int i = 0; i < (n - 1); i++) {
      int menor = i;
      for (int j = (i + 1); j < n; j++) {
        int comp = series[menor].getPais().compareTo(series[j].getPais());
        Comparacoes.cont++;
        if (comp > 0) {
          menor = j;
        } else if (comp == 0 && series[menor].getNome().compareTo(series[j].getNome()) > 0) {
          Comparacoes.cont++;
          menor = j;
        } else {
          Comparacoes.cont++;
...
public class TP03Q01 {
  ...
  public static void main(String[] args) {
    ...
    Arq.openWrite("734661_selecao.txt");
