...
void swap(struct Series *i, struct Series *j)
{
    struct Series temp = *i;
    *i = *j;
    *j = temp;
    //free(&temp);
}

int getMenor(struct Lista *lista, int menor, int j)
{
    if (j == lista->n)
    {
        return menor;
    ...
    if (strcmp(lista->series[menor].pais, lista->series[j].pais) > 0 || ((strcmp(lista->series[menor].pais, lista->series[j].pais) == 0) && strcmp(lista->series[menor].nome, lista->series[j].nome) > 0))
    {
        comp += 2;
        menor = j;
    }

    return getMenor(lista, menor, ++j);
}

void Selecao(struct Lista *lista, int i)
{
    if (i >= (lista->n - 1))
    {
        return;
    }
    int menor = getMenor(lista, i, i + 1);
    if (menor != i)
    {
        swap(&lista->series[menor], &lista->series[i]);
        mov++;
    }
...
void ordenarSelecao(struct Lista *lista)
{
    if (lista->n == 0)
    {
        printf("\nERRO");
...
int main()
{
    ...
    FILE *file;
    file = fopen("734661_selecaoRecursiva.txt", "w");
