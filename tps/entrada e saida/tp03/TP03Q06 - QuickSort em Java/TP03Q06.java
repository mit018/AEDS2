...
class Lista {
  ...
  private void quicksort(int esq, int dir) {
    ...
    Series pivo = series[(dir + esq) / 2];

    while (i <= j) {
      while ((series[i].getPais().compareTo(pivo.getPais()) < 0) || (series[i].getPais().compareTo(pivo.getPais()) == 0 && series[i].getNome().compareTo(pivo.getNome()) < 0)) {
        Comparacoes.cont += 2;
        i++;
      }
      while ((series[j].getPais().compareTo(pivo.getPais()) > 0) || (series[j].getPais().compareTo(pivo.getPais()) == 0 && series[j].getNome().compareTo(pivo.getNome()) > 0)) {
        Comparacoes.cont += 2;
        j--;
      }
      if (i <= j) {
...
public class TP03Q06 {
  ...
  public static void main(String[] args) {
    ...
    Arq.openWrite("734661_quicksort.txt");
