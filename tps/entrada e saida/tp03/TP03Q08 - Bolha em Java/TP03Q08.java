...
class Lista {
  ...
  void ordenarBolha() throws Exception {
    if (n == 0) {
      throw new Exception("ERRO");
    }
    ...
    for (int i = (n - 1); i > 0; i--) {
      for (int j = 0; j < i; j++) {
        if (
          series[j].getTemporadas() > series[j + 1].getTemporadas() ||
          (
            series[j].getTemporadas() == series[j + 1].getTemporadas() &&
            series[j].getNome().compareTo(series[j + 1].getNome()) > 0
          )
        ) {
          Comparacoes.cont += 2;
          swap(j, j + 1);
...
public class TP03Q08 {
  ...
  public static void main(String[] args) {
    ...
    Arq.openWrite("734661_bolha.txt");
