...
class Lista {
  ...
  void ordenarRadixsort() throws Exception {
    if (n == 0) {
      throw new Exception("ERRO");
    }

    int maior = getMaior();
    int digitos = countDigit(maior);
    int casa = 1;
    while (digitos-- > 0) {
      ordenarCountingsort(casa);
      casa *= 10;
    }
  }

  void ordenarCountingsort(int casa) {
    int range = 10; // sistema decimal a ser usado (0 a 9)
    Series[] tmp = new Series[n];
    int[] freq = new int[10];

    // calcula a frequenia dos digitos
    for (int i = 0; i < n; i++) {
      int digito =
        (
          ((series[i].getEpisodios() * 1000) + series[i].getTemporadas()) / casa
        ) %
        range;
      freq[digito]++;
    }

    //Agora, o freq[i] contem o numero de elemento menores ou iguais a i
   ...
   */
  public int getMaior() {
    int maior = (series[0].getEpisodios() * 1000) + series[0].getTemporadas();

    for (int i = 0; i < n; i++) {
      if (
        maior < ((series[i].getEpisodios() * 1000) + series[i].getTemporadas())
      ) {
        maior = (series[i].getEpisodios() * 1000) + series[i].getTemporadas();
      }
    }
...
public class TP03Q10 {
  ...
  public static void main(String[] args) {
    ...
    Arq.openWrite("734661_radixsort.txt");
